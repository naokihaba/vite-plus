...
pub async fn execute(refresh: bool, env_only: bool) -> Result<ExitStatus, Error> {
    ...
    // Generate native Nushell completion file at ~/.vite-plus/vp.nu.
    // This allows env.nu to source completions without requiring Fish to be installed.
    // command_with_help() uses a deep call stack, so we spawn a thread with a larger stack.
    let nu_completions_path: std::path::PathBuf = vite_plus_home.join("vp.nu").as_path().to_path_buf();
    std::thread::Builder::new()
        .stack_size(8 * 1024 * 1024)
        .spawn(move || -> std::io::Result<()> {
            let mut file = std::fs::File::create(&nu_completions_path)?;
            let mut cmd = crate::cli::command_with_help();
            clap_complete::generate(clap_complete_nushell::Nushell, &mut cmd, "vp", &mut file);
            Ok(())
        })
        .map_err(Error::CommandExecution)?
        .join()
        .map_err(|_| Error::Other("Nushell completion generation failed".into()))?
        .map_err(Error::CommandExecution)?;
...
# Load native Nushell completions for vp (generated by clap_complete_nushell).
source "__VP_HOME__/vp.nu"

# Intercept `vp env use` to apply environment changes to the current shell session.
# Overrides the extern defined in vp.nu for execution; vp.nu still provides completions.
def --env --wrapped "vp env use" [...args: string] {
...
export def --env --wrapped vpr [...args: string] { ^vp run ...$args }
"#
    .replace("__VP_BIN__", &bin_path_ref)
...
mod tests {
    ...
    async fn test_create_env_files_nu_contains_path_guard() {
        ...
        assert!(
            nu_content.contains("vp.nu"),
            "env.nu should source the native Nushell completion file"
        );
    }
